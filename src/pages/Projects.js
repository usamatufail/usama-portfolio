              ...
              {filterProjects().map(({ type, name, src, repository, description, site, technologies }, index) => (
                <div key={index} className="card-project">
                  ...
                  <div className="links-container">
                    {repository ? <a href={repository} title="Github" target="_blank" rel="noreferrer"><FiGithub /></a> : null}
                    {site ? <a href={site} title="Ver aplicação" target="_blank" rel="noreferrer"><FiExternalLink /></a> : null}
                  </div>
                </div>
              ))}
            </div>
